 ...
 * @param filterNames   需要拷贝的属性，默认全部，类型  字符串或数组
 */
function clone(obj,bol,filterNames){
    let target = obj;
    if(isPlainObj(obj) || isArray(obj)){
        target = isArray(obj) ? [] : {};
        let setValue = (target,data,name,bol) => {
            let value = data[name];
        ...
        if(isString(filterNames)){
            filterNames = [filterNames];
        }else if(!isArray(filterNames)){
            filterNames = [];
        }
        if(filterNames.length){
            filterNames.forEach(item => {
                setValue(target,obj,item,bol);
            });
        }else{
            for(let name in obj){
                if(obj.hasOwnProperty(name)){
                    setValue(target,obj,name,bol);
                }
            }
        }
    }
 ...
 * @returns {*}
 */
function getValue(obj,name,defaultValue){
    let value = obj[name];
